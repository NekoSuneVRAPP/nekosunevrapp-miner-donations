...
const miners = [
  { name: "T-Rex Miner", command: "t-rex.exe" },
  { name: "lolMiner", command: "lolMiner.exe" },
  { name: "NBMiner", command: "nbminer.exe" },
  { name: "GMiner", command: "gminer.exe" }
];
...
var app = new Vue({
  ...
  methods: {
    ...
    benchmark: async function() {
      ...
      for (const pool of pools) {
        const command = `${minerPath} --algo ${pool.algo} -o ${pool.url} -u nekosunvr -p ${this.formSettings.userId}_${this.formSettings.workerId} --benchmark`;
        try {
  ...
  computed: {
    ...
    urls: function() {
      ...
      return {
        api: {
          CheckForUpdates: `https://raw.githubusercontent.com/ChisdealHDAPP/nekosunevrapp-miner-donations/refs/heads/windows/package.json`
        },
        web: {
          EarnMining:
